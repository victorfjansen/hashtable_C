...
#define DELETED_NODE (struct person *)(0XFFFFFFFFFFFFFFFFFFUL)
...
void print_table()
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        ...
        if (selected_person == DELETED_NODE)
        {
            printf("\t%d\t---<deleted>\n", i);
            continue;
        }
...
int hash_table_insert(struct person *p)
{
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        ...
        if (!hash_table[try_index] || hash_table[try_index] == DELETED_NODE)
        {
...
struct person *hash_table_lookup(char *name)
{
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        ...
        if (foundPerson == DELETED_NODE)
            continue;

        if (!strncmp(foundPerson->name, name, MAX_NAME))
            return foundPerson;
    }
